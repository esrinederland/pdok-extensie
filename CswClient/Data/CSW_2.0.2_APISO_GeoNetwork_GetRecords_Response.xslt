	...
	<xsl:template match="/">
		<Records>
			<xsl:for-each select="/csw:GetRecordsResponse/csw:SearchResults/gmd:MD_Metadata">
              ...
              <References>
				  <xsl:choose>
				  <xsl:when test="count(./gmd:identificationInfo/srv:SV_ServiceIdentification/srv:containsOperations/srv:SV_OperationMetadata[srv:operationName/gco:CharacterString='GetCapabilities'])>0">
				  <xsl:variable name="sUrl" select="./gmd:identificationInfo/srv:SV_ServiceIdentification/srv:containsOperations/srv:SV_OperationMetadata[srv:operationName/gco:CharacterString='GetCapabilities']/srv:connectPoint/gmd:CI_OnlineResource/gmd:linkage/gmd:URL" />
				  <xsl:variable name="sUrlUpper" select="translate($sUrl,'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')" />
<xsl:value-of select="$sUrl"/><xsl:if test="not(contains($sUrlUpper,'?'))"><xsl:text>?</xsl:text></xsl:if><xsl:if test="not(contains($sUrlUpper,'=GETCAPABILITIES'))"><xsl:text>&amp;request=GetCapabilities</xsl:text></xsl:if><xsl:if test="not(contains($sUrlUpper,'=WMS'))"><xsl:text>&amp;service=WMS</xsl:text></xsl:if><xsl:if test="not(contains($sUrlUpper,'VERSION='))"><xsl:text>&amp;version=1.3.0</xsl:text></xsl:if>
				  </xsl:when>
				  <xsl:otherwise>
<xsl:value-of select="gmd:distributionInfo/gmd:MD_Distribution/gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource[gmd:protocol/gco:CharacterString='OGC:WMS']/gmd:linkage/gmd:URL"/>
</xsl:otherwise>
				  </xsl:choose><xsl:text>&#x2714;</xsl:text>urn:x-esri:specification:ServiceType:ArcIMS:Metadata:Server<xsl:text>&#x2715;</xsl:text>
              </References>
